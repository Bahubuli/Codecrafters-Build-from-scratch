...
- **CONCEPT**: transferable systems knowledge, such as Bencode decoding/encoding (strings, integers, lists, dictionaries), SHA-1 info hash verification, metainfo parsing (.torrent files, piece hashes, piece length, tracker URL), HTTP tracker GET requests with URL-encoded binary hashes, peer wire protocol (handshake, bitfield, interested, unchoke, request, piece messages), block-level pipelined piece downloading, piece verification, and multi-peer downloading.
- **SYNTAX / API**: Java names, imports, method calls, `ByteBuffer`, `Socket`, `InputStream`, `OutputStream`, `HttpClient`, `MessageDigest`, and byte array manipulation.
...
5. **Break it**: test partial input, malformed bencode, corrupt SHA-1 hashes, tracker failure codes, peer connection drops, out-of-order blocks, and unchoke state races.
...
- `source`: point to authoritative sources, such as BitTorrent Protocol Specification (BEP 0003) or Java networking documentation.
...
On `reference`, write `Tasks/NN. <Stage Title>.md` with this exact structure.
...
<tables / specs only where relevant - bencode grammar, handshake layout, peer message framing>
...
When operating autonomously to archive BitTorrent solutions for study:
...
1. Check the active stage.
   ...
   ```
   (executed within `.bittorrent-repo`)

2. Implement clean, production-grade Java in `codecrafters-bittorrent-java/src/main/java/Main.java`.
   Verify compilation locally.

   ```powershell
   mvn -f codecrafters-bittorrent-java/pom.xml compile
   ```

3. Create `codecrafters-bittorrent-java/Tasks/NN. <Stage Title>.md` following the pedagogical format: In one line, Self-test, Problem Solved, Concepts, What breaks it (failure modes), Tradeoffs, Wire format / protocol, Java specifics, and Rebuild checklist.
...
5. Submit the committed source to CodeCrafters.
   ...
   ```powershell
   powershell -File scripts\push-bittorrent.ps1 "Stage NN: <Stage Title>"
   ```
...
6. Advance the stage in the web UI and verify the next active task.
   ...
   ```powershell
   python scripts\mark_stage_complete.py bittorrent
   & "C:\Users\jiten\AppData\Local\Programs\codecrafters\codecrafters.exe" task
   ```
