...
- **CONCEPT**: transferable systems knowledge, such as TCP byte streams, HTTP message framing, request parsing, response semantics, connection lifecycles, concurrency, backpressure, and security boundaries.
- **SYNTAX / API**: Java names, imports, method calls, and resource-management shapes.
...
5. **Break it**: test partial input, multiple requests in a stream, malformed headers, missing files, disconnects during writes, concurrent clients, and load where relevant.
...
- `source`: point to authoritative sources, such as RFC 9110 or the Java API documentation.
...
<tables only where relevant - request line, headers, body, response, or framing>
...
When operating autonomously to archive HTTP server solutions for study:
...
2. Implement clean, production-grade Java in `codecrafters-http-server-java/src/main/java/Main.java`.
   Add or extend automated tests first where the project structure supports them.
   Verify compilation locally.

   ```powershell
   javac -d target/classes codecrafters-http-server-java/src/main/java/Main.java
   ```

3. Create `codecrafters-http-server-java/Tasks/NN. <Stage Title>.md` following the pedagogical format: Problem Solved, Concepts, Wire Format, Failure Modes, Tradeoffs, Java Specifics, and Rebuild Checklist.

4. Commit the monorepo change with `Solve Stage NN: <Stage Title>` and push `origin/master`.

5. Submit the committed source to CodeCrafters.

   ```powershell
   powershell -File scripts\push-http.ps1 "Stage NN: <Stage Title>"
   ```
...
6. Advance the stage in the web UI and verify the next active task.
   ...
   ```powershell
   python scripts\mark_stage_complete.py http
   & "C:\Users\jiten\AppData\Local\Programs\codecrafters\codecrafters.exe" task
   ```
