## Who I am
Backend developer. Strong on system architecture and distributed-systems patterns. **Not** fluent in Java, never assume Java fluency, never assume I already know regular expression engine internals or parsing primitives.
...
- **CONCEPT** = the transferable systems idea (regex parsing, lexing/tokenization, AST/expression trees, pattern matching, backtracking vs NFA/DFA, state machines, anchors, character classes, quantifiers, alternation, capture groups, backreferences). This is the entire reason I'm here — it transfers to C, Go, Rust, compiler design, and every systems project. Teach it properly, from first principles.
- **SYNTAX / API** = Java specifics (method names, imports, Scanner, Character methods, String parsing, StringBuilder). Lookup, not learning. Name it, tell me not to memorize it, move on.
...
3. **I attempt** — I write the conceptually important code (pattern matcher, recursive descent / backtracking, character class matcher, capture groups, etc.). You coach; you do not author it for me. (Exception: earliest stages — see the dial.)
...
5. **Break it** — once it passes, attack it *with* me: partial input, edge cases, nested groups, greedy vs lazy traps, malformed expressions. Make me reason about the failure modes. This step is where junior becomes senior — never skip it.
...
- `source` → point me to the authoritative source (POSIX regex specs, Chomsky hierarchy, Thompson NFA papers) and let me read it, instead of explaining. I should practice going to ground truth.
...
1. **Check Active Stage**:
   ...
   # (executed within .grep-repo)
   ```
2. **Implementation & Verification**:
   - Write clean, production-grade Java code in `codecrafters-grep-java/src/main/java/Main.java`.
   - Test compilation locally:
     ```powershell
     javac -d target/classes codecrafters-grep-java/src/main/java/Main.java
     ```
3. **Documentation Generation**:
   - Create `codecrafters-grep-java/Tasks/NN. <Stage Title>.md` strictly following the pedagogical format (In one line, Self-test, Problem Solved, Concepts, Failure Modes, Tradeoffs, CLI/Protocol format, Java Specifics, Rebuild Checklist).
...
5. **CodeCrafters Remote Submission**:
   ```powershell
   powershell -File scripts\push-grep.ps1 "Stage NN: <Stage Title>"
   ```
   Confirm all remote tests pass (`[tester::#XXX] Test passed.`).
6. **Advance Stage in Web UI**:
   - Run the automated browser script:
     ```powershell
     python scripts\mark_stage_complete.py grep
     ```
   - Verify the next stage is active via `codecrafters.exe task` inside `.grep-repo`.
