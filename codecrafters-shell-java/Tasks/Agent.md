## Who I am
Backend developer. Strong on system architecture and distributed-systems patterns. **Not** fluent in Java, never assume Java fluency, never assume I already know an OS or process primitive.
...
- **CONCEPT** = the transferable systems idea (what a shell is; stdin/stdout/stderr; process execution, fork/exec, PATH resolution, builtins vs external commands, quote escaping, redirection, pipes, signal handling). This is the entire reason I'm here — it transfers to C, Go, Rust, every systems project. Teach it properly, from first principles.
- **SYNTAX / API** = Java specifics (method names, imports, ProcessBuilder, Scanner, System.out.print, String parsing). Lookup, not learning. Name it, tell me not to memorize it, move on.
...
3. **I attempt** — I write the conceptually important code (command loop, parser, PATH resolution, builtins, process invocation). You coach; you do not author it for me. (Exception: earliest stages — see the dial.)
...
5. **Break it** — once it passes, attack it *with* me: partial input, unknown commands, weird arguments, multiple spaces, quotes, non-existent binaries. Make me reason about the failure modes. This step is where junior becomes senior — never skip it.
...
- `source` → point me to the authoritative source (POSIX specifications, bash man page, Java ProcessBuilder docs) and let me read it, instead of explaining. I should practice going to ground truth.
...
1. **Check Active Stage**:
   ...
   # (executed within .shell-repo)
   ```
2. **Implementation & Verification**:
   - Write clean, production-grade Java code in `codecrafters-shell-java/src/main/java/Main.java`.
   - Test compilation locally:
     ```powershell
     javac -d target/classes codecrafters-shell-java/src/main/java/Main.java
     ```
3. **Documentation Generation**:
   - Create `codecrafters-shell-java/Tasks/NN. <Stage Title>.md` strictly following the pedagogical format (In one line, Self-test, Problem Solved, Concepts, Failure Modes, Tradeoffs, CLI/Protocol format, Java Specifics, Rebuild Checklist).
...
5. **CodeCrafters Remote Submission**:
   ```powershell
   powershell -File scripts\push-shell.ps1 "Stage NN: <Stage Title>"
   ```
   Confirm all remote tests pass (`[tester::#XXX] Test passed.`).
6. **Advance Stage in Web UI**:
   - Run the automated browser script:
     ```powershell
     python scripts\mark_stage_complete.py shell
     ```
   - Verify the next stage is active via `codecrafters.exe task` inside `.shell-repo`.
7. **Subagent Delegation Loop**:
   ...
   - Subagent prompt template:
     > "You are an autonomous engineering agent solving the next Shell challenge stage.
     > 1. Run `& 'C:\Users\jiten\AppData\Local\Programs\codecrafters\codecrafters.exe' task` in `.shell-repo` to read the active requirements.
     > 2. Implement the solution in `codecrafters-shell-java/src/main/java/Main.java`.
     > 3. Verify compilation with `javac -d target/classes codecrafters-shell-java/src/main/java/Main.java`.
     > 4. Create `codecrafters-shell-java/Tasks/NN. <Title>.md` adhering strictly to `Agent.md`.
     > 5. Commit and push to monorepo: `git add . ; git commit -m 'Solve Stage NN: <Title>' ; git push origin master`.
     > 6. Submit to CodeCrafters: `powershell -File scripts\push-shell.ps1 'Stage NN: <Title>'`.
     > 7. Advance the stage: `python scripts\mark_stage_complete.py shell`.
     > 8. Verify the new task with `codecrafters.exe task` in `.shell-repo` and report results."
