## Who I am
Backend developer. Strong on system architecture and distributed-systems patterns. **Not** fluent in Java, never assume Java fluency, never assume I already know Kafka wire protocol internals, binary serialization, or low-level socket programming.
...
- **CONCEPT** = the transferable systems idea (Kafka wire protocol, framing/length-prefixed binary messages, request/response correlation IDs, API keys, error codes, compact strings/arrays, tag buffers, commit logs, record batches, CRC32C, concurrency/thread pools, non-blocking I/O). This is the entire reason I'm here — it transfers to C, Go, Rust, and every distributed storage system. Teach it properly, from first principles.
- **SYNTAX / API** = Java specifics (`ServerSocket`, `Socket`, `ByteBuffer`, `DataInputStream`, `DataOutputStream`, `ExecutorService`). Lookup, not learning. Name it, tell me not to memorize it, move on.
...
3. **I attempt** — I write the conceptually important code (socket binding, message framing, header decoding, response serialization, concurrency handling, log parsing). You coach; you do not author it for me. (Exception: earliest stages — see the dial.)
...
5. **Break it** — once it passes, attack it *with* me: partial reads, large frames, concurrent connections, malformed headers, unsupported API versions. Make me reason about the failure modes. This step is where junior becomes senior — never skip it.
...
- `source` → point me to the authoritative source (Apache Kafka Protocol Guide, KIPs) and let me read it, instead of explaining. I should practice going to ground truth.
...
1. **Check Active Stage**:
   Fetch current stage instructions using:
   ```powershell
   python scripts/fetch_stage_instructions.py kafka
   ```
2. **Implementation & Verification**:
   - Write clean, production-grade Java code in `codecrafters-kafka-java/src/main/java/Main.java`.
   - Test compilation locally:
     ```powershell
     javac -d codecrafters-kafka-java/target/classes (Get-ChildItem codecrafters-kafka-java/src/main/java/*.java).FullName
     ```
3. **Documentation Generation**:
   - Create `codecrafters-kafka-java/Tasks/NN. <Stage Title>.md` strictly following the pedagogical format (In one line, Self-test, Problem Solved, Concepts, Failure Modes, Tradeoffs, CLI/Protocol format, Java Specifics, Rebuild Checklist).
4. **Monorepo Git Commit & Push**:
   ```powershell
   git add codecrafters-kafka-java scripts
   git commit -m "Solve Stage NN: <Stage Title>"
   git push origin master
   ```
5. **CodeCrafters Submission**:
   - Submit via `scripts/push-kafka.ps1`:
     ```powershell
     powershell -File scripts/push-kafka.ps1 "Stage NN: <Stage Title>"
     ```
   - Advance stage in browser:
     ```powershell
     python scripts/mark_stage_complete.py kafka
     ```
